...
export const bookAppointment = async (req, res) => {
    try {
        ...
        const latestAppointment = await Appointment.findOne({ appointmentDate, doctor: doctorId })
            .sort({ queueNumber: -1 });
        ...
        res.status(201).json({ message: "Appointment booked", newAppointment });
    } catch (error) {
        console.error(error);
...
export const getAppointmentsByDate = async (req, res) => {
    try {
        ...
        const appointments = await Appointment.find({ appointmentDate: new Date(date) })
