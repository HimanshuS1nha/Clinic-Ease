...
export const createUser = async (req, res) => {
    ...
    try {
        ...
        await newUser.save();
        const token = jwt.sign({ id: newUser._id, role: newUser.role }, process.env.JWT_TOKEN, { expiresIn: '1h' });
...
export const getUser = async (req, res) => {
    ...
    try {
        const user = await User.findOne({ $or: [{ _id: id }, { email }, { mobile }] }).select('-password');
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
